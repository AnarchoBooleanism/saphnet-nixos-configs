...
Each Machine generally has its own subdirectory in `<REPO>/machines`, and each subdirectory (e.g. `control-server`) has multiple files involved in the overall NixOS configuration and any services on top of the main configuration. This subdirectly generally includes files such as `version-control.yaml`, which contains NixOS's `stateVersion` and the versions and hashes of relevant non-NixOS packages (e.g. Docker images).
    ...
    { self, nixpkgs, ... } ...: ... {
      nixosConfigurations = {
        "control-server" = nixpkgs.lib.nixosSystem { # control-server is the name of the Instance, which instantiates the mononymous control-server Machine
          ...
          modules = [
            instances/control-server/hardware-configuration.nix # This is the hardware-configuration.nix file for the Machine, auto-generated when nixos-anywhere is started
            ...
            (import machines/control-server/configuration.nix { # This is where the control-server Machine's specific configuration.nix file is imported
              secretsFile = "${./instances/control-server/secrets.enc.yaml}"; # These are the various files imported and used as arguments when importing the configuration.nix file
    ...
    in {
      nixosConfigurations = {
        ...
        "control-server" = nixpkgs.lib.nixosSystem { # Control server for Komodo
          ...
          modules = [
            instances/control-server/hardware-configuration.nix # This should be auto-generated by nixos-anywhere for the Machine
            ...
            (import machines/control-server/configuration.nix { # The Machine is instantiated here, being turned into a NixOS configuration that can be taken in by nixpkgs.lib.nixosSystem itself
              secretsFile = "${./instances/control-server/secrets.enc.yaml}"; # These are the Instance-specific values used to instantiate the Machine
              instanceValues = builtins.fromTOML (builtins.readFile "${./instances/control-server/instance-values.toml}"); 
              constantsValues = builtins.fromTOML (builtins.readFile "${./constants/homelab-constants-values.toml}"); 
            })
          ];
...
Here is an example of `secrets.enc.yaml`, for no machine in particular (as cleartext):
